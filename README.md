...
We learned where to use an ACL, and we found a way to implement it.
An ACL is one of the most useful **Context Mapping** strategies, if
not the most important one. There are other strategies to discover...

## Step 3

Sometimes we can find a service which is duplicated in many systems. It
contains the same rules in every one of them. For example, if your are
informed that all checks found in the HR System are also implemented in
some other systems by other development teams.
